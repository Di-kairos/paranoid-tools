...
### Changed
- **Парольный контейнер получил версию `SSPP2` и 600 000 итераций PBKDF2** (аудит 2026-09-07,
  §16.2 — пропуск исходного аудита, найденный при разборе). Собравший порог долей перебирает
  passphrase офлайн, а бумага управляет тем, где лежат доли, а не тем, как быстро их
  переберут; OWASP для PBKDF2-HMAC-SHA256 называет 600 000, у нас было 200 000. Число итераций
  `openssl enc` в заголовке не хранит, поэтому оно едет вместе с магией: новые контейнеры —
  `SSPP2` с `-md sha256 -iter 600000` (дайджест закреплён явно, чтобы контейнер открывался и на
  чужой машине через годы), `SSPP1` по-прежнему **читается** с 200 000, голый `Salted__` — тоже.
  Доля, уже лежащая на бумаге, не переписывается и работать не перестаёт. Windows-порт
  распознаёт обе магии и выдаёт команду расшифровки с правильным для версии числом итераций —
  неверное число выглядело бы как неверный пароль. Шаг 200k → 600k стоит атакующему втрое
  больше работы (≈1,6 бита): усиливает нормальный пароль и не спасает слабый.
- **Формулировка про тег приведена к тому, что он есть.** 16-байтовый sha256-тег внутри
  шифротекста — проверка неверного пароля (≈2⁻¹²⁸), а не MAC: подлинность против переписывания
  долей он не доказывает, для этого нужен AEAD или Encrypt-then-MAC с разделением ключей.
  README EN/RU переписаны без слова «аутентифицирован» в сильном смысле.

...
setup() {
  ...
  WORK="$(mktemp -d)"
  FIX="${WORK}/release"        # «релиз»: seedsplit + SHA256SUMS
  DEST="${WORK}/bin/seedsplit"
  mkdir -p "$FIX" "${WORK}/bin"
  printf '#!/usr/bin/env bash\necho payload-ok\n' > "${FIX}/seedsplit"
  ( cd "$FIX" && shasum -a 256 seedsplit > SHA256SUMS )
}
...
@test "install.sh installs binary when checksum matches" {
  ...
  run env PATH="$BINS" SEEDSPLIT_BASE_URL="file://${FIX}" SEEDSPLIT_DEST="$DEST" ALLOW_UNSIGNED_LEGACY=1 bash "$INSTALL"
  [ "$status" -eq 0 ]
  [ -x "$DEST" ]
  run "$DEST"
  [[ "$output" == *"payload-ok"* ]]
...
@test "install.sh fails closed when .sig is absent (no ALLOW_UNSIGNED_LEGACY)" {
  BINS="$(_make_fakebin with-keygen)"
  run env PATH="$BINS" SEEDSPLIT_BASE_URL="file://${FIX}" SEEDSPLIT_DEST="$DEST" bash "$INSTALL"
  [ "$status" -ne 0 ]
  [ ! -e "$DEST" ]
}
...
@test "install.sh installs next to an existing umbrella copy instead of making a second one" {
  ...
  printf '#!/usr/bin/env bash\necho stale\n' > "${FAKEHOME}/.local/bin/seedsplit"
  chmod +x "${FAKEHOME}/.local/bin/seedsplit"
  run env PATH="$BINS" HOME="$FAKEHOME" SEEDSPLIT_BASE_URL="file://${FIX}" ALLOW_UNSIGNED_LEGACY=1 bash "$INSTALL"
  [ "$status" -eq 0 ]
  run "${FAKEHOME}/.local/bin/seedsplit"
  [[ "$output" == *"payload-ok"* ]]
}

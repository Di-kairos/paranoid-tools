...
### Changed
- **Windows: `vaultwatch start` сам поднимает UAC.** Лаунчер и трей запрашивают права за
  пользователя с s43, `securetrash vault ...` — со своей командной строки; прямой
  `vaultwatch start` при этом отказывал и советовал открыть консоль администратора руками.
  Теперь неэлевированный запуск показывает один запрос прав и заводит сессию в отдельном окне
  с правами; окно держится до Enter, чтобы вывод дочитался. Код возврата элевированного
  прогона становится нашим. Fail-closed не тронут: без прав сессия по-прежнему НЕ пишется.
  Отказ остаётся ответом там, где запрос отклонён или его некому показать: хук `securetrash`,
  задача планировщика, CI получают прежний быстрый отказ. Выключается `ST_NO_SELF_ELEVATE=1`.

#!/usr/bin/env bash
# vendor-common.sh — embeds securetrash lib/common.sh inline into the vaultwatch file
# between markers. The source is pinned to a securetrash git ref AND to a SHA256 of the
# contents (defense-in-depth: a ref can be rewritten/MITMed — the hash catches byte tampering).
#
# Usage:
#   tools/vendor-common.sh          # refresh the embedded block from the source (network needed)
...
# Take common.sh from the monorepo (exact bytes) and verify the SHA256. Missing
# file/hash failure → exit 3 (NOT "drift": CI must tell a source failure from desync).
_fetch_common_to() {
  local out="$1" actual
...
# common.sh, END, everything after END. Bytes are embedded via cat (no trailing-newline trimming).
build_expected() {
  local commonfile="$1"
...
if [[ "${1:-}" == "--check" ]]; then
  # OFFLINE: hash the embedded block and compare with the pinned SHA256 — no network. CI does
  ...
  # Runs only when the sibling source exists: a standalone copy of the tool
  # (or a test sandbox) has no monorepo layout — the block check below still
  # guards the bytes there. In monorepo CI the source is always present.
  if [[ -f "$SRC_FILE" ]]; then
  ...
  # UPDATE: pull the exact bytes of common.sh from the pinned ref (network needed) and check the SHA.
  COMMON_FILE="$(mktemp)"

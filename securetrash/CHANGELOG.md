# Changelog

Все заметные изменения securetrash. Формат — [Keep a Changelog](https://keepachangelog.com/ru/1.1.0/),
версионирование — [SemVer](https://semver.org/lang/ru/).

## [Unreleased]

### Fixed
- **Windows: `vault open` на подключённом, но ЗАПЕРТОМ контейнере больше не отвечает «Уже
  открыт» (живой прогон, s46–s48).** После паники том заперт, а контейнер остаётся присоединён;
  ветка идемпотентности видела «присоединён» и называла это открытым — человек шёл в Проводник
  и получал «Access is denied». Теперь вердикт тот же, что у `vault status`: запертый контейнер
  отключается и открывается по-настоящему (с паролем), незашифрованный — отказ с прежним
  предупреждением, и только подключённый И разблокированный — «Уже открыт».
- **Windows: подключённый контейнер БЕЗ буквы диска больше не называется «Уже открыт».**
  Такой контейнер остаётся после открытия, прерванного между присоединением и выдачей буквы
  (`Mount-DiskImage -NoDriveLetter`), и после паники в том же промежутке. Добраться до тома
  нечем — что бы про него ни сказал BitLocker, это не «открыт»; раньше проба шла по букве,
  отвечала «неизвестно», и `open` отвечал «Уже открыт» поверх недостижимого тома. Теперь он
  отключается и открывается заново, как запертый, и заканчивает нормальной буквой и sidecar'ом.
  `vault status` в том же случае спрашивает BitLocker по пути тома (`\\?\Volume{...}\`), а не по
  букве из sidecar'а — та от прошлого монтирования и могла достаться уже чужому тому.
- **Windows: приостановленный BitLocker больше не выдаётся за незашифрованный том.**
  У приостановленного (и у шифруемого/расшифровываемого) тома `ProtectionStatus` = `Off` при
  том, что данные — шифротекст; вердикт «НЕ зашифрован» советует уничтожить контейнер и создать
  заново, то есть предлагал стереть настоящие данные. Теперь «незашифрован» говорится только
  про `FullyDecrypted`.
- **Windows: `vault open` больше не показывает «Access is denied», чужой запрос «Unlock drive» и
  красный `diskpart failed` на удавшемся открытии (живой прогон, s48).** `diskpart attach vdisk`
  отдавал ещё ЗАПЕРТОМУ тому запомненную букву сразу: Проводник открывал том, который не мог
  прочитать, BitLocker показывал собственный тост «Unlock drive E:» рядом с нашим промптом, а
  наш `assign letter=` гонялся с автоматической буквой и проигрывал — открытие проходило со
  второй попытки. Теперь контейнер подключается **без буквы** (`Mount-DiskImage -NoDriveLetter`),
  `Unlock-BitLocker` идёт по пути тома, и только разблокированный том получает букву — Windows
  выбирает свободную и назначает её одним шагом, так что гонки «выбрал — назначил» больше нет.
  По итогам adversarial-ревью: раздел данных выбирается по типу (MBR IFS / GPT Basic) и обязан
  быть ровно один — контейнер с другой раскладкой отвергается, а не разблокируется «какой первый»;
  если букву выдать не удалось, том сперва запирается и лишь потом отключается, а неудавшееся
  отключение называется вместе с путём тома, а не сводится к «нет свободной буквы».
- **Windows: `check` читает состояние BitLocker за миллисекунду и без прав (живой прогон, s48).**
  `Get-BitLockerVolume` без прав администратора ждёт около пяти секунд на WMI-провайдере и лишь
  затем отказывает — `check` из трея, в обычном окне, платил это ожидание, чтобы напечатать
  «неизвестно, перезапусти от администратора». Теперь первым спрашивается свойство Проводника
  `System.Volume.BitLockerProtection` (1 и 6 — включён, 2 — выключен): ответ есть у любого
  пользователя, промежуточные состояния (шифруется, расшифровывается, приостановлен) честно
  отдаются как «неизвестно», а к командлету путь остаётся только при отсутствии ответа (Home).
  `setup` и `shred` читают то же трёхзначное состояние, так что с `check` они больше не расходятся.
- **Windows Home: сейф недоступен по редакции, и теперь это сказано прямо (аудит 2026-09-07, §9
  и §16.6).** Управление BitLocker (`manage-bde` и командлеты) идёт в Pro/Enterprise/Education,
  в Home его нет — а `check` показывал «состояние BitLocker определить не удалось» и советовал
  «включи BitLocker», то есть отправлял искать несуществующую галочку и выглядел как временный
  сбой. Появился разбор причины: `Get-StWindowsEdition` читает `EditionID` из реестра, и отказ
  называет редакцию ТОЛЬКО когда реестр действительно говорит Home (`Core*`); в остальных
  случаях — «командлетов нет, но редакция не Home: компонент не установлен либо закрыт
  политикой», потому что отсутствие командлета само по себе Home не доказывает. Home при этом
  не объявляется незашифрованной: Device Encryption на системном диске возможна, она просто не
  даёт команд, на которых построен сейф. Причина печатается в трёх местах: `check`, отказ
  `vault create`, отказ `vault open`. Требование редакции описано в README EN/RU.

### Changed
- **Windows: прямой `securetrash vault ...` сам поднимает UAC.** С s43 права запрашивают за
  пользователя и лаунчер, и трей; вызов той же команды из обычной консоли при этом отказывал и
  советовал закрыть окно и открыть PowerShell правой кнопкой. Теперь неэлевированный запуск
  `vault create|open|close|destroy|reset|destroy-old` показывает один запрос прав и выполняет
  команду в отдельном окне с правами — окно держится открытым до Enter, чтобы вывод дочитался,
  а пароль сейфа было куда ввести. Код возврата элевированного прогона становится нашим.
  Отказ никуда не делся: он остаётся ответом, когда запрос отклонён или его некому показать —
  неинтерактивный вызов (планировщик, CI, скрипт в конвейере) по-прежнему получает быстрый
  отказ, а не зависание на невидимом диалоге. Выключается `ST_NO_SELF_ELEVATE=1`.
  Сообщение `check` о консоли без прав приведено в соответствие: команды не «откажутся»,
  а спросят права.

## [0.5.8] — 2026-09-02

### Fixed
- **Windows: промпты пароля сейфа больше не называют чужую машинерию.** «Enter BitLocker password
  to unlock the vault» живой пользователь прочитал как системный BitLocker или пароль Windows и
  спросил, какой именно вводить. У промпта пароля нет задачи объяснять, чем контейнер зашифрован
  внутри: теперь он говорит, что это пароль ЭТОГО сейфа, заданный при его создании, и прямо
  исключает пароль Windows. Промпт создания тоже назван так, чтобы было видно — пароль новый.

- **Windows: подтверждение нового пароля сравнивается в неуправляемой памяти.**
  `PtrToStringBSTR` отдавал две managed-копии пароля обычными `System.String`; строки
  неизменяемы, затереть их нечем — они просто лежали в куче до сборки мусора, которой в
  живущем процессе могло и не случиться. BSTR-ы зануляются в `finally` и раньше; теперь
  плейнтекст не покидает их вообще: сравнение идёт посимвольно по указателям.
- **Windows: `paranoid` больше не советует `brew upgrade`.** Homebrew — только macOS; строка
  подсказки в Windows-лаунчере называет реальный путь: `git pull` в клоне и `windows\install.cmd`.

### Added
- **Windows: `check` и вывод после `shred` называют MFT-резидентные копии.** Файл примерно до
  700 байт (а seed-фраза и ключ именно такие) на NTFS не имеет собственных блоков: содержимое
  лежит внутри записи MFT, `cipher /w` перезаписывает свободные кластеры и туда не добирается.
  Из userland это не чинится — значит, это надо назвать, как названы SSD и снапшоты.

- **Windows: команды `vault` отказываются в консоли без прав администратора — вместо сырого падения diskpart.**
  Весь сейф работает через `diskpart` и командлеты BitLocker, а их Windows отдаёт только
  администратору. Без прав `vault open` падал выводом diskpart, а `vault create` успевал дойти
  до `Enable-BitLocker` и оставить подключённый НЕЗАШИФРОВАННЫЙ том на месте сейфа. Теперь
  проверка стоит один раз, до любых вопросов и подтверждений: команда отказывается, ничего не
  тронув, и называет нужную консоль. `status` не гейтится (чтение — не привилегия), но если
  состояние не прочитать без прав, он говорит именно это, а не «определить не удалось».
  `check` отдельной строкой сообщает, потянет ли эта консоль сейф вообще.
- **Windows: `securetrash` не запускался из PowerShell — «running scripts is disabled on this system».**
  Установщик клал `securetrash.ps1` рядом с `securetrash.cmd` в каталоге из PATH, а PowerShell разрешает
  имя команды без расширения в `.ps1` раньше, чем в `.cmd` с тем же именем, и тут же отказывается
  его грузить при стандартной политике запуска (`Restricted`). То есть обёртка, ради которой всё
  и затевалось, не получала управления никогда — кроме вызова из `cmd.exe`. Теперь сам скрипт
  лежит в подкаталоге `lib\`, в PATH видна только `.cmd`, и она стартует
  `pwsh -NoProfile -ExecutionPolicy Bypass` для своего процесса: политика машины не меняется.
  Установка поверх старой убирает `securetrash.ps1`, оставшийся рядом с обёрткой, — иначе он бы
  продолжал перехватывать имя, и обновление ничего бы не изменило.

### Added
- **`vault create` спрашивает новый пароль дважды и предупреждает о коротком — но не запрещает его.**
  Раньше пароль читался один раз и без единой проверки: опечатка запирала контейнер навсегда
  (сброса у сейфа нет by design), а пустой ввод проходил молча. Теперь ввод идёт **циклом**:
  пароль короче порога — предупреждение о том, чем это грозит, и вопрос «всё равно использовать?»;
  ответил «нет» — вводишь заново, а не начинаешь весь `create` сначала. Пароли не совпали — тоже
  заново, а не отказ. Выход из цикла — пустой ввод (ничего не создаётся).
  Порог `ST_VAULT_PASS_MIN` (12 по умолчанию, `0` выключает) — это граница ПРЕДУПРЕЖДЕНИЯ, а не
  правило: измерителем стойкости длина не является, реальное сопротивление офлайн-перебору даёт
  энтропия (5-6 слов diceware), см. `THREAT-MODEL.md`. Инструмент, который перебивает решение
  владельца о его же секрете, — это инструмент, который начинают обходить.
  Проверка стоит только на создании: `open` берёт пароль как раньше, иначе существующий сейф
  с коротким паролем стал бы недоступен. `ST_VAULT_PASS` (test-only хук для CI/Pester/smoke)
  обходит диалог целиком — подтверждать в неинтерактивном запуске не с чем.
  Порт под Windows повторяет логику один в один (`Get-StVaultPasswordNewSecure`, SecureString end-to-end).

## [0.5.7] — 2026-08-13

### Changed
- **Проект переехал в монорепозиторий [`paranoid-tools`](https://github.com/Di-kairos/paranoid-tools).**
  Код, история и релизы всех пяти инструментов живут теперь в одном репозитории; релизные теги
  получили префикс — этот выпуск опубликован как `securetrash-v0.5.7`. Прежний репозиторий архивирован,
  его старые релизы продолжают отдаваться.
- **Установщики качают из монорепо по закреплённому тегу.** `releases/latest` в монорепо означает
  «последний релиз любого инструмента», поэтому и `install.sh`, и `windows/install.ps1` всегда
  пиннят собственный тег `securetrash-vX.Y.Z`; версию по умолчанию сторожит гейт в `release.yml`.
- **Комментарии в коде переведены на английский** для внешних контрибьюторов; строки интерфейса
  остаются двуязычными, русская документация — на месте.
- **Windows-установщик говорит на одном языке.** Сообщения об отказе (несовпадение
  контрольной суммы, отсутствие `ssh-keygen`, отсутствие или провал подписи) были
  русскими, тогда как остальной вывод `install.ps1` — английский; теперь весь скрипт
  английский, как и его happy path.
- Приватные сообщения об уязвимостях принимаются теперь в одном месте —
  [advisories монорепо](https://github.com/Di-kairos/paranoid-tools/security/advisories/new).

### Fixed
- **Windows: `vault create` больше не оставляет хранилище открытым.** `diskpart` отдаёт VHDX
  подключённым, BitLocker — разблокированным, и созданный сейф стоял открытым, пока
  пользователь не вспомнит его закрыть. Теперь `create` отсоединяет контейнер, как это
  всегда делал macOS (`hdiutil create` образ не подключает): сейф открывается только
  явной командой `vault open` с паролем. Шифрование включается с `-UsedSpaceOnly` — на
  только что отформатированном пустом томе стирать нечего, зато большой контейнер не
  шифруется фоном минутами, оставаясь всё это время разблокированным.
- **Windows: `vault status` при неизвестном состоянии возвращает ненулевой код** — раньше
  текст говорил «определить не удалось», а код возврата сообщал скрипту об успехе.
- **macOS: `vault status` больше не выдаёт «ЗАКРЫТ» за неизвестное состояние.** Если `hdiutil`
  недоступен или его вывод не разобрался, состояние не установлено — теперь так и говорится,
  с ненулевым кодом возврата. Раньше `_vault_dev` схлопывал `unknown` в пустую строку, и
  успокаивающий ответ выдавался без основания. Windows-порт различал эти три состояния всегда.
- **Windows: провалившийся `vault create` больше не оставляет незашифрованный том.**
  `Enable-BitLocker` падает уже после того, как `diskpart` создал, подключил и отформатировал
  VHDX, — на диске оставался открытый NTFS-том, который `vault status` показывал как
  `[ok] Container is OPEN`. Теперь неудача отсоединяет и удаляет полусозданный контейнер, как
  это давно делает macOS-ветка. Найдено первым живым прогоном на Windows 11.
- **Windows: `vault status` больше не считает «подключён» синонимом «открыт».**
  Подключённый, но заблокированный том (неверный пароль) и подключённый незашифрованный том
  теперь называются своими именами; вердикт `OPEN` остаётся только для тома с включённой
  защитой BitLocker.
- **Windows: размер меньше минимума BitLocker (64 МБ) отклоняется до `diskpart`**, а не
  падает исключением `0x8031006F` после создания контейнера.
- **Windows: `check` про BitLocker говорит, чего не хватает.** Без прав администратора
  `Get-BitLockerVolume` просто отказывает — вместо тупикового «could not determine status»
  выводится подсказка про запуск от администратора (так же, как это давно сделано для
  теневых копий).

### Added
- В README появилась строка принадлежности к экосистеме Paranoid Tools; лендинг ссылается на
  монорепозиторий.

## [0.5.6] — 2026-08-08

### Fixed
- **Скрипт снова запускается под штатным Windows PowerShell 5.1.** Файлы `.ps1` лежали в UTF-8
  без BOM, а 5.1 читает такой файл в системной ANSI-кодировке: на русской Windows кириллица в
  исходнике превращалась в мозаику, кавычки внутри строк рвались, и скрипт падал с `ParserError`
  ещё до первой команды — не работало вообще ничего. Дефект нашёл живой пользователь; CI его не
  видел, потому что гонял только `pwsh` 7, который читает UTF-8 по умолчанию. BOM добавлен во все
  `.ps1`, включая установщик.

### Added
- **CI гоняет тесты под самим Windows PowerShell 5.1**, а не только под `pwsh` 7: парсинг каждого
  `.ps1`, Pester и запуск `securetrash version` / `--help`. Плюс тест, который падает, если хоть один `.ps1` с не-ASCII
  остался без BOM.


## [0.5.5] — 2026-08-06

### Fixed
- **Установщик Windows работает и под штатным Windows PowerShell 5.1.** Аргументы верификатору
  подписи собирались через `ProcessStartInfo.ArgumentList`, которого в .NET Framework нет, —
  а 5.1 это ровно тот шелл, в котором выполняют однострочник из README. Установка падала бы на
  шаге проверки подписи. Добавлен запасной путь через строку аргументов с корректным
  квотированием (кавычки экранируются, хвостовые обратные слэши удваиваются).

## [0.5.4] — 2026-08-06

### Fixed
- **Установщик больше не плодит вторую копию тула.** Умбрелла `paranoid-tools` ставит всё в
  `~/.local/bin` (без sudo), а этот установщик по умолчанию — в `/usr/local/bin`. Кто ставил
  обоими способами, получал две копии, и какая из них запустится, решал порядок в `PATH`:
  обновление молча не доезжало до пользователя. Теперь при уже установленной копии в
  `~/.local/bin` установка идёт поверх неё; явный `<TOOL>_DEST` по-прежнему сильнее всего.
  Плюс предупреждение, если каталог установки не в `PATH`, — молчать здесь нельзя, иначе
  успешная установка выглядит как несостоявшаяся.

## [0.5.3] — 2026-08-06

### Fixed
- **Установщик Windows больше не может тихо повиснуть на проверке подписи.** `stdout`/`stderr`
  верификатора перенаправлялись, но не вычитывались до `WaitForExit`: `ssh-keygen`, написавший
  больше буфера трубы, вставал на записи, а установщик ждал его вечно. Молча висящая установка
  хуже честного отказа. Потоки теперь дренируются асинхронно; регрессия воспроизведена тестом
  (без дренажа установщик не возвращается).
- **Установщик Windows честно говорит «подпись не сошлась», если верификатор вышел, не дочитав
  данные.** Запись в уже закрытую трубу роняла установку необработанным исключением — вместо
  вердикта пользователь видел аварию.

## [0.5.2] — 2026-08-05

### Fixed
- **`check` больше не докладывает «FileVault ВЫКЛЮЧЕН», когда `fdesetup` недоступен.**
  Отсутствие команды читалось как «шифрование выключено» — инструмент врал пользователю
  о его защите ровно в том месте, где к нему приходят за правдой. Теперь такой случай
  честно называется «не удалось определить».

### Changed
- **`lib/common.sh`: один ответ на вопрос «сейф примонтирован?» для всей экосистемы.**
  Библиотека получила `_volume_mounted` (читает таблицу монтирования, а не наличие
  каталога — остаточный `/Volumes/…` от прошлого монтирования больше не выглядит открытым
  сейфом) и `_fv_state` (FileVault on/off/**unknown**; `filevault_on` теперь его
  двоичная обёртка). Точка монтирования сравнивается целиком, поэтому смонтированный
  `/Volumes/Foo Bar` не выдаёт себя за `/Volumes/Foo`. Код возврата `_volume_mounted` —
  трёхзначный: 0 примонтирован, 1 точно нет, 2 таблицу прочитать не удалось; предикатным
  вызовам двойка читается как «нет» (fail-closed), а тому, кто показывает состояние
  человеку, — как «не знаем». Имена зарезервированы для вендоринга (см. шапку файла).

## [0.5.1] — 2026-08-05

### Added
- **Windows-порт догнал macOS.** Раньше пользователь, переходящий между платформами,
  упирался в то, что выглядело как баги: `vault status` в `securetrash.ps1` не было
  вовсе (теперь есть, read-only: OPEN с реальной точкой монтирования, CLOSED или —
  в отличие от bash, где такого режима отказа нет — честное «состояние определить не
  удалось», если Storage-командлеты молчат); `-v/--version` и `-h/--help` отвечали
  «Unknown command»; размеры не принимали суффиксы hdiutil (`5g`, `500m`), которые
  понимает bash — теперь принимают и переводят в мегабайты diskpart с округлением
  вверх, а абсурдно длинное число отклоняется как неверный размер, а не переполняется.

### Fixed
- **Windows: вывод больше не идёт через `Write-Host`.** Из-за этого
  `securetrash check > report.txt` и лаунчер (`windows/paranoid.ps1`) не получали
  ничего. Информационные строки уходят в stdout, предупреждения и ошибки — в stderr,
  как в bash и в остальных четырёх портах.
- **Прерванный `vault create` больше не оставляет мёртвый контейнер.** Нет места,
  отказ hdiutil, Ctrl-C на вводе пароля — оставался `.sparsebundle`, который не
  открывался ни одним паролем, но выглядел как сейф: следующий `create` отказывал
  («уже существует»), а `destroy` предлагал уничтожить всё — ради пустой оболочки.
  Удаляется только контейнер, созданный этим же вызовом; уже существовавший путь не
  трогается, поэтому `reset` (он сначала отставляет старый в сторону) не задет.
- **`install.ps1`: `SHA256SUMS` отдаётся верификатору сырыми байтами.** Подписанные
  данные шли через конвейер PowerShell, который перекодирует текст — BOM или замена
  переводов строк превращали верную подпись в «incorrect signature» и блокировали
  установку.
- **Заранее выставленный `ST_LOCALE` уважается на обеих платформах.** securetrash был
  единственным из пяти, кто определял локаль безусловно, — и хост, уже её
  разрешивший (лаунчер `paranoid`, GUI), получал смешанный вывод: обвязка
  по-русски, securetrash по-английски. Теперь оба порта следуют контракту вендоренного
  `common.sh`: унаследованный `ST_LOCALE` побеждает, детект — только фолбэк.
- «Unknown command» переведено в русской таблице (последняя английская строка в ней).

### Changed
- README: `empty` — это не «Empty» из лаунчера. Одно очищает `~/SecureTrash`,
  перезаписывая файлы на месте (best-effort, без гарантий на SSD), другое — `vault
  reset`, крипто-шреддинг зашифрованного контейнера. Одно слово, две разные
  разрушительные операции.
- README EN+RU: VeraCrypt на Windows — ручной фолбэк, а не автоматизированный. Код
  сознательно отказывается им управлять: его CLI принимает пароль в argv, где его
  прочитает любой другой процесс, поэтому `vault` обнаруживает установленный VeraCrypt
  и отправляет в его собственный GUI.

## [0.5.0] — 2026-08-03

### Added
- **`vault destroy-old`** — штатный способ убрать контейнер, отставленный прерванным
  `reset`. Раньше инструмент сообщал, что рядом лежит `<vault>.old` с прежними данными,
  но убрать его можно было только руками. Отдельная команда, а не часть `destroy`: цели
  разные, и смешивать их в одном подтверждении — прямой путь удалить не то. Состояние
  «размонтирован» проверяется дважды — до промпта и непосредственно перед удалением,
  потому что подтверждение ждёт ввода сколько угодно, а сам инструмент советует
  примонтировать контейнер и забрать данные.
- **`check` и заметка после `shred`/`empty` показывают локальные снимки.** Снимок,
  сделанный пока файл ещё лежал вне сейфа, хранит его полную копию, и `shred` туда не
  дотянется. Считаются все снимки APFS кроме откатных точек установщика: список только
  по `com.apple.TimeMachine.*` пропустил бы Carbon Copy Cloner и Arq и выдал бы
  «снимков нет» там, где копия есть. На Windows — теневые копии VSS, и только при правах
  администратора: без них провайдер отдаёт пустой список, а не ошибку.

### Fixed
- **Окно потери данных в `vault reset` закрыто.** Раньше `destroy` шёл первым, и падение
  `create` (нет места, отказ hdiutil/diskpart, Ctrl-C на промпте пароля, отказ
  Enable-BitLocker) оставляло пользователя вообще без сейфа. Теперь старый контейнер
  отставляется в сторону, новый создаётся на его месте, и только после успеха старый
  крипто-шредится. Откат навешен trap'ом (EXIT/INT/TERM/HUP/QUIT), а не только на ветку
  ошибки: прерывание приходит именно на промпте пароля, уже после переноса. Уцелевший
  `.old` — данные пользователя, поэтому `reset` отказывается его перезаписывать, а не
  сносит молча. Восстановление на место не выполняется, если путь занят: `mv dir dir`
  положил бы старый сейф внутрь недоделанного нового и вернул успех.
- **Windows: суффикс отставленного контейнера идёт перед расширением**
  (`SecureVault.old.vhdx`). `Get-DiskImage` принимает только `.vhd/.vhdx/.iso`, поэтому
  при `SecureVault.vhdx.old` любая проверка состояния отвечала бы `unknown` и
  fail-closed отказом — контейнер нельзя было бы ни проверить, ни удалить.
- **Провал crypto-shred больше не молчит.** Если старый контейнер удалить не удалось,
  рядом с новым сейфом остаётся копия, расшифровываемая старым паролем; теперь об этом
  сказано прямо, а не выдано за успешный reset.
- **`Get-CimInstance Win32_ShadowCopy` получил `-OperationTimeoutSec 10`** — зависший
  VSS/WMI иначе подвешивал бы каждый `shred`.

### Notes
- Тесты: bats 81 → 96, Pester 86 → 109. Добавлены реально-файловые тесты пары
  aside/restore (моки скрывали два Windows-дефекта) и проверка отказа при смонтированном
  `.old`.

## [0.4.13] — 2026-07-09

### Fixed
- **Windows: `check` различает BitLocker `off` и `unknown`** — зеркало macOS-фикса v0.4.12.
  Раньше отсутствие cmdlet'а `Get-BitLockerVolume` (Windows Home) или неопределяемый статус
  трактовались как «ВЫКЛЮЧЕН»; теперь tri-state `Get-StBitLockerState` печатает честное
  «BitLocker: неизвестно — считай, что диск НЕ защищён». Булев `Get-StBitLockerOn`
  (guard в setup/rm-путях) не тронут. Pester +1.
- **windows-ci реанимирован (красный с v0.4.11):** фейковый ssh-keygen в тестах
  установщика стал `ssh-keygen.cmd` (файл без расширения Windows не исполняет —
  happy-path supply-chain теста падал fail-closed'ом), destroy-тесты получили
  default-мок `Test-Path` под strict-режим Pester 6. Поведение установщика и CLI
  не менялось — только тест-инфраструктура. bats 78/78 + Pester 76/76.

## [0.4.12] — 2026-07-06

### Fixed
- **`check` теперь различает FileVault `off` и `unknown`.** Раньше вердикт был бинарным
  (`filevault_on`: On / не-On), поэтому недоступный или непонятный вывод `fdesetup` трактовался
  как «ВЫКЛЮЧЕН» — и расходился с дашбордом лаунчера, который показывал «неизвестно». Теперь
  `check` использует tri-state (`on`/`off`/`unknown`, как у лаунчера): при неопределённом статусе
  печатает «FileVault: неизвестно — считай, что диск НЕ защищён» вместо ложного «OFF».
  Вендоренный примитив `filevault_on` не тронут. bats 72 → 73.

## [0.4.11] — 2026-07-04

### Added
- **Windows-установщик проверяет Ed25519-подпись релиза** — `ssh-keygen -Y verify` поверх
  `SHA256SUMS`, fail-closed. Отказ от проверки возможен только через `PT_ALLOW_HASH_ONLY=1`
  (при отсутствии верификатора или файла подписи).
- **`vault reset` задокументирован** в таблицах команд (macOS README + Windows README).

### Security
- **`VAULT_PATH`/`VAULT_VOLUME` учитывают `ST_VAULT_PATH`/`ST_VAULT_VOLUME`** — GUI/TUI и
  деструктивный CLI теперь бьют по одному и тому же сейфу (раньше могли указывать на разные).
- **`vault reset` валидирует аргумент размера ДО уничтожения** контейнера (нет частичного
  разрушения при неверном размере).
- **Windows `vault open` идемпотентен** — повторный вызов на уже открытом сейфе безопасен.

### Changed
- Смягчены формулировки про `shred` — соответствуют честному рантайм-поведению на SSD.
- Исправлен version-drift в документации.

## [0.4.10] — 2026-06-27

### Added
- **`securetrash vault reset [size]`** — опустошить сейф с РЕАЛЬНОЙ гарантией безвозвратности:
  crypto-shred контейнера (выкинуть ключ) + создать новый пустой (новый ключ → старое мертво
  навсегда). Покрывает воркфлоу «накопил файлы → бах, гарантированно стёрто → работаю дальше».
  Почему не in-place: перезапись файлов внутри живого сейфа на SSD — best-effort (тот же ключ
  продолжает расшифровывать остаточные band-блоки); единственный честный способ — выкинуть ключ.
  Один confirm на всю операцию; запрос пароля нового сейфа. Размер нового контейнера опционален.
- Windows-порт: `vault reset` зеркалит ту же логику (VHDX destroy+recreate, fail-closed по
  unmounted-состоянию).

### Changed
- Рефактор `cmd_vault`: общие внутренние операции (`_vault_create_now`/`_vault_assert_destroyable`/
  `_vault_destroy_now`) переиспользуются `destroy` и `reset` (DRY). Поведение create/destroy не менялось.

## [0.4.9] — 2026-06-26

### Changed
- **`vault open` теперь монтирует том ВИДИМЫМ в Finder по умолчанию** (убран `-nobrowse`).
  Причина: спрятанный том после закрытия окна «невидим» — пользователь не понимает, где сейф,
  и легко забывает его смонтированным; а открытый vault «под угрозой» — это бóльший риск, чем
  сама видимость в сайдбаре. Видимость даёт и штатный выход: «извлечь» из Finder автоматически
  размонтирует том. Старое приватное поведение (вне сайдбара/рабочего стола) — opt-in через
  `ST_VAULT_HIDDEN=1`. На Windows том всегда был виден (буква диска) — поведение сошлось.
  NB: при «извлечь» из Finder post-close хук securetrash не срабатывает (vaultwatch-исключения
  снимутся при следующем явном `vault close` или по TTL) — на безопасность не влияет.

## [0.4.8] — 2026-06-26

### Changed
- **Windows-паритет vault reveal.** `securetrash.ps1`: после монтирования контейнера том
  открывается в Explorer (`explorer.exe <drive>:\`) — зеркало macOS `open <mountpoint>` из
  0.4.7. Reveal best-effort (ошибка запуска Explorer не роняет успешный open) и идёт после
  post-open хука. Отключить: `ST_VAULT_NO_REVEAL=1` (та же переменная, что и на macOS).

## [0.4.7] — 2026-06-26

### Changed
- **`vault open` теперь открывает том в Finder.** Раньше образ монтировался с `-nobrowse`
  (приватность — том не висит в боковой панели / на рабочем столе), и открытый сейф было
  попросту не видно: пользователь не понимал, куда класть файлы. Теперь после монтирования
  выполняется явный `open <mountpoint>` — окно сейфа всплывает, как при двойном клике по dmg,
  а `-nobrowse` по-прежнему держит том вне сайдбара. Отключить: `ST_VAULT_NO_REVEAL=1`.

## [0.4.6] — 2026-06-26

### Changed
- Репозиторий переехал на `github.com/Di-kairos`; перевыпуск с обновлёнными URL и переподписанными ассетами. Функциональных изменений нет.

## [0.4.5] — 2026-06-25

Исправление Windows-интеграции экосистемы: vault-хуки на Windows теперь действительно срабатывают.

### Fixed
- **Windows vault hooks (F1):** `securetrash vault open/close` на Windows раньше НЕ запускал
  хуки `~/.securetrash/hooks/post-open.cmd` / `post-close.cmd`, хотя `vaultwatch install-hooks`
  их туда клал, а доки обещали авто-охрану. Открытый vault оставался без присмотра (Windows
  Search-исключения и TTL-автозакрытие не включались). Порт приведён к контракту macOS
  (`_run_vault_hook`): хук запускается только если файл есть; его падение лишь предупреждает
  и не роняет vault-операцию. Пост-монтажные действия best-effort — ошибка sidecar/хука не
  превращает успешный `open` в провал.
- **Активный том в sidecar (F1/F2):** securetrash выбирает первую свободную букву динамически;
  теперь при `open` она пишется в `<vault>.vhdx.mount` (ACL-защищён, хранит только букву диска —
  не секрет), читается при `close` для post-close хука и лаунчером `paranoid` для корректного
  определения смонтированного тома. Чистится при `close`/`destroy`.

### Tests
- +6 Pester: post-open/post-close hook dispatch, жизненный цикл mount-sidecar, no-op хука без
  файла, override `ST_HOOK_DIR`.

## [0.4.4] — 2026-06-24

Полиш-релиз: паритет постуры установщика с sibling-тулами + чистка публичного репозитория.

### Security
- **`install.sh`:** при вшитом pubkey, но отсутствии `ssh-keygen` установщик теперь громко
  предупреждает, что подпись НЕ проверена (только целостность по SHA256), вместо молчаливого
  пропуска. Паритет с vaultwatch/panic/ghostdraft/seedsplit.

### Housekeeping
- Из публичного репозитория убраны dev-kitchen артефакты (внутренние `CLAUDE.md`, `ECOSYSTEM.md`,
  `PROGRESS.md`, `docs/promo/`, `docs/sessions/`) — теперь gitignored, в релизный tarball не попадают.

## [0.4.3] — 2026-06-23

### Fixed
- **Release signing fail-closed:** выпуск прерывается, если `RELEASE_SIGNING_KEY` не задан —
  релиз без подписи больше невозможен (раньше молча откатывался до checksum-only).
- **`install.sh` fail-closed на отсутствие `.sig`:** установщик жёстко отказывает, если подпись
  релиза отсутствует (для легаси-релизов до v0.4.2 — явный `ALLOW_UNSIGNED_LEGACY=1`).
- **Windows `shred` — reparse-point guard:** junction/symlink удаляется как запись, без рекурсии
  в target.
- **Windows-порт: синхронизирована версия** (`securetrash.ps1` отставал — был 0.4.1 при Bash 0.4.2).

## [0.4.2] — 2026-06-23

### Fixed
- **`shred` — guard на mount-root:** отказывается шредить `/Volumes/<name>` (прямые потомки
  `/Volumes`) — защищает внешние диски и точку монтирования самого vault от случайного
  рекурсивного удаления. Раньше защищён был только сам `/Volumes`.
- **`install.sh` — pubkey подписи релиза:** ассеты v0.4.1 собрали до встраивания pubkey;
  `RELEASE_SIGNING_PUBKEY` был пуст, авто-проверка молча пропускалась. Ассеты релиза v0.4.2
  содержат правильный pubkey — проверка подписи в установщике работает.

## [0.4.1] — 2026-06-22

### Added
- **Подпись релизов (Ed25519, опциональная):** CI подписывает `SHA256SUMS`, `install.sh`
  авто-проверяет подпись поверх контрольной суммы. Мягкая деградация — нет ключа/подписи
  не ломает установку. Pubkey опубликован в `SECURITY.md`.
- Флаги-алиасы `-v`/`--version` и `-h`/`--help`.
- Канонический `lib/common.sh` — источник вендоринга для экосистемы Paranoid Tools.
- Хуки `vault open/close` (`post-open`/`post-close`) — точка интеграции vaultwatch/panic.

### Fixed
- **Windows `vault destroy` — fail-closed (tri-state):** не удаляет backing-файл, пока том
  смонтирован или состояние не определено; mounted → размонтировать и перепроверить.
- **Windows `shred` — protected-path guard:** отказ для корней дисков и системных деревьев
  (Windows, Program Files, ProgramData, корень Users, профиль).

### Changed
- **Windows: честный tri-state детект диска** (ssd/hdd/unknown) — неизвестный тип больше не
  выдаётся за HDD с обнадёживающим «перезапись помогает».
- Честная формулировка crypto-shred в `windows/README`; landing-install тянется с релизного
  тега (а не с подвижной `main`) с проверкой.

## [0.4.0] — 2026-06-18

### Added
- **Checksum-verified `install.sh`** (F-2): бинарь и `SHA256SUMS` тянутся с релизного
  тега, хеш проверяется ДО установки — закрывает supply-chain риск «curl|bash из main».

### Changed
- **`vault destroy` — fail-closed:** никогда не удаляет, пока контейнер смонтирован или
  состояние неизвестно (защита от случайного уничтожения живого vault).
- Hardening: закреплён mountpoint vault, самохостинг шрифтов в доках, FAQ про угрозу
  открытого vault.

## [0.3.0] — 2026-06-17

### Added
- **PowerShell-порт под Windows (beta)** с Pester-тестами и CI.

### Fixed
- Security-hardening: честные формулировки гарантий, `--` guard'ы на путях, проверка
  целевого тома, права `0700`, флаг `--yes`.
- Windows: пароли не через argv, валидация diskpart, `-LiteralPath`, разблокировка
  BitLocker, отслеживание backend.
- Де-пиннинг version-теста.

## [0.2.0] — 2026-06-17

### Added
- **i18n:** вывод по умолчанию на английском, русский — через `ST_LANG=ru`.
- Демо-gif и английский README/guide как основной (русский — вторичный).

## [0.1.0] — 2026-06-17

Первый публичный срез: безопасное удаление + шифрованный vault (crypto-shred) для macOS.

### Added
- **`check`** — честный аудит окружения (FileVault, тип диска SSD/HDD) и вердикт, какие
  гарантии реальны на данном железе.
- **`vault create|open|close|destroy|status`** — шифрованный контейнер; «уничтожение» =
  крипто-шреддинг (удаление ключа), что реально на SSD в отличие от перезаписи.
- **`shred`**, **`setup`**, **`empty`** — удаление файлов/папок, инициализация `~/SecureTrash`.
- README, LICENSE, гайд.

### Honest limitations
- На SSD перезапись (`rm -P`) гарантий НЕ даёт (wear leveling, COW, TRIM) — для секретов
  использовать `vault` превентивно. Подробности — `README.md` «Scope & limitations».

[Unreleased]: https://github.com/Di-kairos/paranoid-tools/compare/securetrash-v0.5.8...HEAD
[0.5.8]: https://github.com/Di-kairos/paranoid-tools/compare/securetrash-v0.5.7...securetrash-v0.5.8
[0.5.7]: https://github.com/Di-kairos/paranoid-tools/releases/tag/securetrash-v0.5.7
[0.5.5]: https://github.com/Di-kairos/securetrash/compare/v0.5.4...v0.5.5
[0.5.4]: https://github.com/Di-kairos/securetrash/compare/v0.5.3...v0.5.4
[0.5.3]: https://github.com/Di-kairos/securetrash/compare/v0.5.2...v0.5.3
[0.5.2]: https://github.com/Di-kairos/securetrash/compare/v0.5.1...v0.5.2
[0.5.1]: https://github.com/Di-kairos/securetrash/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/Di-kairos/securetrash/compare/v0.4.13...v0.5.0
[0.4.13]: https://github.com/Di-kairos/securetrash/compare/v0.4.12...v0.4.13
[0.4.12]: https://github.com/Di-kairos/securetrash/compare/v0.4.11...v0.4.12
[0.4.11]: https://github.com/Di-kairos/securetrash/compare/v0.4.10...v0.4.11
[0.4.10]: https://github.com/Di-kairos/securetrash/compare/v0.4.9...v0.4.10
[0.4.9]: https://github.com/Di-kairos/securetrash/compare/v0.4.8...v0.4.9
[0.4.8]: https://github.com/Di-kairos/securetrash/compare/v0.4.7...v0.4.8
[0.4.7]: https://github.com/Di-kairos/securetrash/compare/v0.4.6...v0.4.7
[0.4.6]: https://github.com/Di-kairos/securetrash/compare/v0.4.5...v0.4.6
[0.4.5]: https://github.com/Di-kairos/securetrash/compare/v0.4.4...v0.4.5
[0.4.4]: https://github.com/Di-kairos/securetrash/compare/v0.4.3...v0.4.4
[0.4.3]: https://github.com/Di-kairos/securetrash/compare/v0.4.2...v0.4.3
[0.4.2]: https://github.com/Di-kairos/securetrash/compare/v0.4.1...v0.4.2
[0.4.1]: https://github.com/Di-kairos/securetrash/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/Di-kairos/securetrash/releases/tag/v0.4.0
[0.3.0]: https://github.com/Di-kairos/securetrash/releases/tag/v0.3.0
[0.2.0]: https://github.com/Di-kairos/securetrash/releases/tag/v0.2.0
[0.1.0]: https://github.com/Di-kairos/securetrash/releases/tag/v0.1.0

...
### Fixed
- **Windows: черновик в консоли заканчивается одиночной точкой на отдельной строке (живой
  прогон, s47).** Договор «Ctrl-Z на пустой строке, затем Enter» на живой машине не сработал:
  консоль напечатала `^Z` как текст, черновик не завершался, и человек через минуту набрал
  одну точку на своей строке, пытаясь выйти. Точка — то, за что хватается застрявший, поэтому
  точка и завершает черновик; Ctrl-Z + Enter по-прежнему работает там, где работает. Подсказка
  называет точку первой, Ctrl-Z — в скобках. Тот же урок, что выход из vim на macOS: застрявшего
  освобождает механика, а не формулировка.

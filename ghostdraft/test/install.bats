...
@test "install.sh installs next to an existing umbrella copy instead of making a second one" {
  ...
  printf '#!/usr/bin/env bash\necho stale\n' > "${FAKEHOME}/.local/bin/ghostdraft"
  chmod +x "${FAKEHOME}/.local/bin/ghostdraft"
  run env PATH="$BINS" HOME="$FAKEHOME" GHOSTDRAFT_BASE_URL="file://${FIX}" ALLOW_UNSIGNED_LEGACY=1 bash "$INSTALL"
  [ "$status" -eq 0 ]
  run "${FAKEHOME}/.local/bin/ghostdraft"
  [[ "$output" == *"payload-ok"* ]]
}

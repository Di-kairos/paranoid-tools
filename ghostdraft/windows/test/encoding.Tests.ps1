...
# mojibake, quotes and brackets inside strings break, and the script dies with a
# ParserError before the first command. PowerShell 7 reads UTF-8 by default and thus
# never sees the problem — CI stayed green until a real user ran `securetrash check`
# on their machine and got "Unexpected token 'РїРѕРґ'" instead of a working tool.
#
...
Describe 'ps1 encoding' {
    It 'every .ps1 with non-ASCII bytes starts with a UTF-8 BOM' {
        ...
        # and a `$bad +=` inside it would be lost along with the test result.

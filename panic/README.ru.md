...
- **«Мгновенно» никто не обещает — запуск измеряется, и замер честно говорит, что в него
  входит.** Шаги здесь — внешние команды (`hdiutil detach`, блокировка экрана), общей гарантии
  по времени у них нет: занятый том, зависшая системная команда или отсутствующий механизм
  блокировки займут столько, сколько займут. `panic now` печатает по монотонным часам, сколько
  заняло отсоединение и когда была запрошена блокировка. Отсчёт идёт *внутри* команды, то есть
  всё, что было до неё, в него не входит: терминал, оболочка, а на Windows — диалог UAC, который
  ждёт человека и верхней границы не имеет. Если вызывающий передал `--trigger-ms <epoch_ms>`
  (так делают приложение в меню-баре, трей Windows и лаунчер), вторая строка показывает весь путь
  от нажатия. Не передал — строки просто нет, вместо выдуманного числа.
- **Строка про блокировку заявляет запрос, а не запертый экран.** `LockWorkStation` на Windows
  и блокировка на macOS возвращаются, как только запрос принят; сам экран система рисует чуть
  позже и о моменте не сообщает. Поэтому инструмент говорит, что блокировка запрошена и принята,
  и просит взглянуть на экран: честный глагол не стоит ничего, а неверный стоит доверия.

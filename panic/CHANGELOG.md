...
### Added
- **Хоткей паники в трее Windows — `Ctrl+Alt+P` дважды (s48, решение Mr.Di).** «Четыре клавиши
  одновременно, очень быстро, два раза — слишком большая комбинация»: было `Ctrl+Alt+Shift+P`.
  Shift-варианты остались в настройках; файл настроек со старым дефолтом переезжает один раз с
  уведомлением, сознательно выбранная старая комбинация сохраняется. Оговорка в коде и
  `gui/README.md`: Windows отдаёт AltGr как Ctrl+Alt, и на раскладке, где AltGr+P печатает символ
  (US-International: ö), два таких символа за 2 с сработают как паника — там выбирай Shift-вариант.
- **Трей Windows умеет автозапуск с правами администратора (Task Scheduler, `RunLevel Highest`).**
  Из `HKCU\Run` трей стартует обычным пользователем, и тогда каждое действие сейфа и `panic now`
  стоят диалога UAC — включая срабатывание хоткея паники, то есть ровно тот момент, когда диалог
  неуместен. Через задачу трей поднимается уже с правами, и хоткей срабатывает без диалога.
  Цена написана на самом пункте меню и в подтверждающем уведомлении: всю сессию трей держит
  права администратора, и что скомпрометирует его — скомпрометирует с ними. Поэтому по умолчанию
  выключено, вынесено отдельным пунктом (а не третьим состоянием обычного автозапуска), и само
  включение/выключение стоит одного UAC. Два автозапуска взаимоисключающие.

### Measured
- **Приёмка §16.4 аудита снята на живой Windows (Parallels VM, трей с правами, без UAC, s49).**
  30 прогонов подряд + холодный: от триггера до запроса блокировки экрана **p50 1,97 с, p95
  2,07 с, max 2,11 с** при потолке аудитора p95 ≤ 3 с и max ≤ 5 с. Внутри прогона: тома закрыты
  за 1,21 с (p50), запрос блокировки — 1,65 с, старт pwsh до первого действия — 0,32 с. 30/30
  без отказов: том заперт, контейнер отсоединён, `attached_after=False`. Отдельные исходы:
  **без прав** (×5) паника честно сообщает, что сейф остался открыт, и не рисует успех;
  **занятый том** (открытый файл на сейфе) закрывается принудительно за 1,88 с. Двойной хоткей
  из GUI в цифры не входит — он снимается руками. Мерялось `panic now --trigger-ms`: метка
  времени ставится перед запуском, так что в цифру входит и старт процесса, а не только наш код.

### Fixed
- **Windows: после `panic now` контейнер сейфа отсоединяется, а не остаётся висеть запертым
  (D3 живого прогона, s46).** `Lock-BitLocker` только запирал том, а VHDX оставался присоединён:
  Проводник продолжал рисовать `SecretVault (E:)`, и человек читал это как «паника не сработала».
  На macOS `hdiutil detach -force` отсоединяет каждый смонтированный образ — теперь Windows
  делает то же: после запирания все файловые виртуальные диски (`Get-Disk`, BusType
  `File Backed Virtual`) отсоединяются через `Dismount-DiskImage`, отчёт называет их число
  отдельной строкой, `panic status` показывает, что будет отсоединено. Без прав — как и
  BitLocker: не пробуем и не ждём, а `status` так и говорит («или нет прав посмотреть»).
- **Windows: отчёт паники читается по порядку — предупреждения больше не обгоняют его (D6
  живого прогона, s46).** Предупреждения шли в stderr, отчёт — в stdout; лаунчер запускает
  panic дочерним процессом через конвейер, и stderr ребёнка долетал до консоли раньше, чем
  stdout через трубу: строки склеивались (`...administrator PowerShell[+] process start...`),
  а точка от предупреждения висела отдельной строкой. Теперь `[!]`-строки — часть отчёта и
  идут тем же потоком в том порядке, в каком случились; в stderr остались только фатальные
  `[x]` (неизвестная команда, usage).
- **Windows: без прав `panic now` больше не тратит пять секунд на доказательство того, что
  права уже сказали (живой прогон, s46).** Приёмка по §16.4 аудита дала 8,84 с от нажатия при
  потолке 5 с, из них 5,29 с — на «закрытие нуля томов»: `panic now` без прав ходил в
  `Get-BitLockerVolume`, а тот отвечает `Access denied` за 5,23 с холодного WMI. Когда
  `Test-PnElevated` уже ответил «нет», туда не ходим. Стало: без прав 3,45 с (наш код 1,21), с
  правами 6,87 с (наш код 1,84, остальное — UAC и человек у диалога). Отчёт делит время на «до
  того, как процесс существовал» и «наше», а ожидание перед процессом больше не приписывается
  Windows целиком.
- **Отказ в UAC больше не оставляет машину как была (живой прогон Windows, s45).** Лаунчер и трей
  после «No» не делали НИЧЕГО — включая блокировку экрана, которая прав не требует. Для сейфа
  такой отказ честен (без diskpart/BitLocker он не откроется), для паники — противоположность
  смыслу кнопки. Теперь при отказе panic запускается без прав: буфер, история буфера, recents и
  блокировка экрана выполняются, а в отчёте и в уведомлении прямо сказано, что шифр-тома
  остались открытыми. Команды сейфа при отказе по-прежнему не делают ничего.

### Changed
- **Замер `panic now` покрывает весь путь, а не только собственный хвост (аудит 2026-09-07,
  §16.4).** Секундомер стартовал внутри команды — после терминала, запуска процесса и диалога
  UAC, то есть мерил более короткую и более лестную половину. Теперь: внутренние шаги идут по
  монотонным часам (NTP может утащить настенные назад посреди паники и сделать длительность
  отрицательной), Windows отдельной строкой печатает время от старта процесса до первого
  действия, а вызывающий передаёт момент нажатия — `--trigger-ms <epoch_ms>` (флаг переживает
  элевированный перезапуск) или `PANIC_TRIGGER_MS`. Момент проставляют меню-бар macOS, трей
  Windows и лаунчер; именно эта строка содержит ожидание UAC, у которого верхней границы нет.
  Не передали — строки нет: отсутствующее число лучше выдуманного, скачок часов отбрасывается.
- **Строка блокировки больше не заявляет запертый экран.** `LockWorkStation` и блокировка macOS
  возвращаются, как только запрос принят, — экран система рисует позже и о моменте не сообщает.
  Вместо «экран заперт» — «блокировка запрошена и принята, взгляни на экран». README EN/RU тоже.

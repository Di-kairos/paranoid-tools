...
### Честные утилиты приватности и безопасности для macOS и Windows — каждая делает одно дело, без шарлатанства.
...
один файл-скрипт (чистый Bash на macOS, PowerShell-порт на Windows) с
...
| 5 | [`seedsplit`](https://github.com/Di-kairos/seedsplit)     | разбить секрет на доли (Шамир) + парольная фраза | macOS · Windows (beta) | [![latest](https://img.shields.io/github/v/release/Di-kairos/seedsplit?display_name=tag&label=&color=2ea44f)](https://github.com/Di-kairos/seedsplit/releases/latest) |
...
сборка релизов, и обязательный раздел **Scope & limitations** — прочитай его, прежде чем
...
На свежем клоне каждый инструмент тянется из своего **подписанного релиза** по принципу
«проверь, потом запусти»: установщик проверяет подпись Ed25519 над `SHA256SUMS`, затем контрольную
сумму install.sh самого инструмента и только потом запускает его — а тот, в свою очередь,
проверяет бинарь до установки. То есть каждый скачанный из сети артефакт — install.sh
конкретного инструмента и его бинарь — проверяется до запуска (сам верхний `bash install.sh`
ты запускаешь руками, прочитав его). Закрепить версию — `PT_PANIC_VERSION=0.1.7`; сменить
каталог — `PT_DEST=/usr/local/bin`.
...
```bash
bash install.sh --uninstall   # снять все инструменты и лаунчер
```

### Windows

Однострочный `install.sh` выше — только для macOS. На Windows это несколько коротких
шагов, вот всё с нуля. Шаги **1–2 делаются один раз**; шаг 3 повторяешь для каждого инструмента.

**1. Поставь PowerShell 7.** Поддерживаемый путь установки и запуска — PowerShell 7 (`pwsh`);
встроенный в Windows PowerShell 5.1 официально не поддерживается. В любом терминале (нажми
`Win`, набери «PowerShell», Enter):
...
**Коротко:** подпись защищает канал доставки, но единый ключ на все пять репо остаётся общей
точкой риска.
...
обновлении изменилось поведение во время работы (напр. `securetrash vault` теперь монтирует том
...
сама тебя не найдёт, ты проверяешь её сам. Самый дешёвый и не нарушающий приватность способ: на GitHub нажми
...
тоже может показать строку «доступно обновление» на дашборде (если включишь) — см.
[Лаунчер](#лаунчер).
...
Открытый сейф всегда помечается «под угрозой».
...
**Проверка обновлений (по желанию).** По умолчанию ВЫКЛ — дашборд не трогает сеть, пока ты сам
не попросишь. Задай `PARANOID_UPDATE_CHECK=1`, и дашборд добавит строку *«доступно обновление»*,
когда у установленного инструмента есть более свежий подписанный релиз. Это единственный
сетевой вызов лаунчера: один запрос на инструмент к GitHub `releases/latest` (следует за
редиректом, без API-ключа, без телеметрии), с кэшем на 24 часа. Включить на сессию — `PARANOID_UPDATE_CHECK=1
...
- **Раздельные репозитории + вендоринг.** Общий код — канонический `securetrash/lib/common.sh`;
  ...
  Скрипт синхронизации и CI-проверка дрейфа держат копии честными. Ни зависимостей во время работы, ни шага сборки.
...
Донаты идут на поддержку и необязательный слой удобства (нативный menu-bar / tray, Фаза B).

...
> **Статус: ранний (v0.1.9, work in progress).** Готовы интеграция (хуки + вендоринг),
...
> [модель угроз](https://github.com/Di-kairos/paranoid-tools/blob/main/THREAT-MODEL.ru.md). Для воспроизводимости фиксируй версию: `VW_VERSION=0.1.9` вместо `latest`.

> Текущий публичный релиз — **v0.1.9** (подписан, с `install.sh` + `SHA256SUMS`).
> Для воспроизводимости фиксируй его: `VW_VERSION=0.1.9` вместо `latest`.

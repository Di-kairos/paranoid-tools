# Pester 5: every .ps1 with non-ASCII bytes must start with a UTF-8 BOM.
#
...
# mojibake, quotes and brackets inside strings break apart, and the script dies with a
# ParserError before the first command even runs. PowerShell 7 reads UTF-8 by default and
# therefore never sees the problem — CI stayed green until a live user ran
# `securetrash check` on their machine and got "Unexpected token 'РїРѕРґ'" instead of a working tool.
#
...
Describe 'ps1 encoding' {
    It 'every .ps1 with non-ASCII bytes starts with a UTF-8 BOM' {
        ...
        # exactly two positional parameters, and the third kills the test with a ParameterBindingException.
        $root = Resolve-Path (Join-Path (Join-Path $PSScriptRoot '..') '..')
        $bad = @()
        # foreach, not ForEach-Object: the latter executes in a child scope,

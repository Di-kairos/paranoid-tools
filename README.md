...
> **Status: early (v0.1.11, work in progress).** Done: integration (hooks + vendoring),
...
> [threat model](https://github.com/Di-kairos/paranoid-tools/blob/main/THREAT-MODEL.md). Pin a version with `VW_VERSION=0.1.11` instead of `latest`
> for reproducibility.

> The current public release is **v0.1.11** (signed, with `install.sh` + `SHA256SUMS`).
> Pin it for reproducibility with `VW_VERSION=0.1.11` instead of `latest`.
